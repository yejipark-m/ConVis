...
class MPLUGOWL2(BaseModel):
    ...
    def generate(
        ...
        image = samples["image"]
        if convis_decoding:
            image = torch.cat([image, gen_images], dim=0)
        bs = image.size(0)
        ...
        input_ids = [tokenizer_image_token(p, self.llm_tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt') for p in prompt]
        ...
        output_text, tokens_lists = self.llm_tokenizer.decode(outputs[0, input_ids.shape[1]:])
        output_text = [output_text.strip()]
        return [outputs, output_text, tokens_lists]
