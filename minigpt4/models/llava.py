...
class LLaVa(BaseModel):
    ...
    def generate(
        ...
        output_text, tokens_lists = self.llama_tokenizer.batch_decode(output_ids[:, input_token_len:], skip_special_tokens=True)
        output_text = [text.split('###')[0].strip() for text in output_text]
        return [output_ids, output_text, tokens_lists]
