...
question_path = data_path + f"/questions.jsonl"

generated_image_file_path = args.images_path + f"/{model_name}/num_steps_{args.diff_num_step}/"

base_dir = os.path.join(output_dir, "llava-bench", args.model)
...
with open(question_path, 'r', encoding='utf-8') as question_list:
    ...
    for idx, line in enumerate(tqdm(lines, unit="line")):
        ...
        image_path = os.path.join(data_path, "images", img_file)
        ...
        if convis_decoding:
            pattern = generated_image_file_path + f"image_{img_file.split('.')[0]}*"
        ...
        dict_save["prompt"] = qu_raw
        dict_save["answer_id"] = img_file
        dict_save["model_id"] = model_name
        dict_save["metadata"] = {}
        dict_save["text"] = output_text
