...
class GenerationMixin:
    ...
    def generate(
        ...
        convis_decoding: Optional[bool] = None,
        ...
        **kwargs,
        ...
        elif convis_decoding and generation_mode == GenerationMode.GREEDY_SEARCH:
            print("\033[41m!!!!! ConVis Decoding !!!!!!\033[0m")
            return self.convis_greedy_search(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
    ...
    def convis_greedy_search(
        ...
        synced_gpus: bool = False,
        streamer: Optional["BaseStreamer"] = None,
        convis_alpha: Optional[float] = 0.0,
        cutoff_lambda: Optional[float] = None,
        **model_kwargs,
    ) -> Union[GreedySearchOutput, torch.LongTensor]:
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        ...
        while True:
            ...
            ########################################################
            """
            ConVis's Implementation
            """

            original_logit = next_token_logits[0]
            manipulated_logits = next_token_logits[1:]
            manipulated_logit = manipulated_logits.mean(dim=0)

            cd_logits = (1 + convis_alpha) * original_logit - convis_alpha * manipulated_logit

            if cutoff_lambda is not None:
                original_logit = self.relative_top_filter(original_logit, cutoff_lambda)
                mask = (original_logit == float("-inf"))
                cd_logits[mask] = -1e3
    ...
    def opera_beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            current_state["beam_indices"] = beam_indices[:] if beam_indices is not None else None
        ...
        beam_hyp_score = torch.tensor([beam[0] for beam in beam_scorer._beam_hyps[0].beams])
        final_beam_hyp_idx = torch.argmax(beam_hyp_score)
        final_beam_indices = list(beam_scorer._beam_hyps[0].beams[final_beam_hyp_idx][2])
        final_beam_indices = torch.tensor(final_beam_indices)
